-- ================================================
-- SISTEMA DE ROLES E ADMINISTRADORES
-- Para controle de acesso administrativo
-- ================================================
...
-- FUNÇÃO PARA CONCEDER ROLE DE ADMIN
...
-- FUNÇÃO PARA REMOVER ROLE DE ADMIN
...
-- CONCEDER ADMIN PARA O USUÁRIO INICIAL (dolescfo@gmail.com)
-- Execute apenas UMA VEZ após criar as tabelas e funções
/*
INSERT INTO user_roles (user_id, role, granted_by)
SELECT 
    u.id, 
...
-- Conceder admin para um usuário:
-- SELECT grant_admin_role('novo.admin@cirurgia.com.br');

-- Remover admin de um usuário:
-- SELECT revoke_admin_role('usuario@email.com');

-- Ver roles de um usuário específico:
-- SELECT ur.role, ur.granted_at, gb.email as granted_by
-- FROM user_roles ur
-- LEFT JOIN auth.users gb ON ur.granted_by = gb.id
-- WHERE ur.user_id = (SELECT id FROM auth.users WHERE email = 'dolescfo@gmail.com');
